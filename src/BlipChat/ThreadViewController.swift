...
internal class ThreadViewController: UIViewController, WKNavigationDelegate, UIScrollViewDelegate, WKUIDelegate {
  ...
  override func viewDidLoad() {
    ...
    let backButton = UIImage(named:"backButton", in: resourcesBundle, compatibleWith: nil);
    var cancelButton: UIBarButtonItem? = nil;

    if (backButton != nil) {
      cancelButton = UIBarButtonItem(image: backButton, style: .plain, target: self, action: #selector(ThreadViewController.handleCancel))
    } else {
...
blipClient.sendMessage({ \
  \"type\": \"text/plain\", \
  \"content\": \"\(self.options.initialMessage ?? "")\" \
});
"""
    }
