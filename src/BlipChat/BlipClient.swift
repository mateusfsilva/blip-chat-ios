...
@objc public class BlipClient : NSObject{
  ...
  @objc public static func openBlipThread(myView: UIViewController, appKey: String, options: BlipOptions = BlipOptions()) throws{
    ...
    do {
      try validateSdkConfiguration(appKey: appKey, options: options)
    } catch {
      throw error
    }
  ...
  internal static func validateSdkConfiguration(appKey: String, options:BlipOptions) throws {
    if appKey.isEmpty {
